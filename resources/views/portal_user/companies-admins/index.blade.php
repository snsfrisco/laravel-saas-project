...
@section('title') {{ __('Companies Admins') }} @endsection

@section('breadcrumb')
    <li class="breadcrumb-item"><a href="{{ route('portal_user.dashboard') }}">{{ __('Dashboard') }}</a></li>
    <li class="breadcrumb-item active">{{ __('Companies Admins') }}</li>
@endsection

@section('add-model-button')
    {{-- @can('create_user') --}}
        <a href="{{ route('portal_user.companies-admins.create') }}" class="btn btn-primary btn-sm float-right">
            <i class="fa fa-plus"></i> {{ __('Add Company Admin') }}
        </a>
    {{-- @endcan --}}
@endsection
...
@section('content')
    <div class="card card-primary card-outline">
        <div class="card-header">
            <h3 class="card-title">{{ __('Companies Admins') }}
    ...
    <script>
        $(function() {
            $('#data-table').DataTable({
                ...
                fixedHeader: true,
                stateSave: true,
                ajax: "{{route('portal_user.companies-admins.index')}}",
                columns: [
